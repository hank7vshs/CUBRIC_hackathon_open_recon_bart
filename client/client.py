...
defaults = {
    ...
    'ui_data':         'debug_ui_data', # conf file to emulate not existing fields in send datasets
    'config_local':   '',
...
def main(args):
    ...
    local_ui_data_text = None
    if (args.ui_data):
        ui_data_file = args.ui_data + '.json'
        if os.path.exists(ui_data_file):
            logging.info("Found additional config file with UI data %s", ui_data_file)

            fid = open(ui_data_file, 'r')
            local_ui_data_text = fid.read()
            fid.close()
    ...
    if (args.config_local):
        ...
        logging.info("Sending remote config file name '%s'", args.ui_data)
        connection.send_config_file(args.ui_data)
    ...
    groups = dset.list()
    if ('xml' in groups):
        mrd_header_data = dset.read_xml_header()
        mrd_header_data = mrd_header_data.decode("utf-8")
    else:
        logging.warning("Could not find MRD metadata xml in file")
        mrd_header_data = "Dummy XML header"
    connection.send_metadata(mrd_header_data)

    # --------------- Send additional config -----------------------
    groups = dset.list()
    if local_ui_data_text is None:
        if ('configAdditional' in groups):
            ui_data_text = dset._dataset['configAdditional'][0]
            ui_data_text = ui_data_text.decode("utf-8")
            logging.info("Sending configAdditional found in file %s:\n%s", args.filename, ui_data_text)
            connection.send_text(ui_data_text)
        else:
            # Do nothing -- no additional config in local .json file or in MRD file
            pass
    else:
        if ('configAdditional' in groups):
            logging.warning("configAdditional found in file %s, but is overriden by local file %s!", args.filename, ui_data_file)
        logging.info("Sending configAdditional found in file %s:\n%s", ui_data_file, local_ui_data_text)
        connection.send_text(local_ui_data_text)
    ...
    dset.write_xml_header(bytes(mrd_header_data, 'utf-8'))
    dset.close()

    logging.info("---------------------- Summary ----------------------")
    logging.info("Sent %5d acquisitions  |  Received %5d acquisitions", connection.sentAcqs,      recvAcqs.value)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-u', '--ui-data',                             help='Path to UI data JSON file (without .json extension); overrides the default debug_ui_data.json')
