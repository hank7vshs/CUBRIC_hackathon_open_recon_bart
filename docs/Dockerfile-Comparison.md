...
| **Pinned Python packages** | `h5py==3.12.1`, `ismrmrd==1.14.1`, `matplotlib==3.9.2`, `pydicom==3.0.1`, `numpy==2.4.2`, `check-jsonschema==0.33.3`, `pypulseq` | `numpy==2.4.2`, `matplotlib==3.9.2`, `ismrmrd` (**version currently unpinned** — worth aligning to `1.14.1` at some point) |
...
> `matplotlib==3.9.2` (not `3.10.8`) is pinned pre-emptively in both files — `Supervised_RNS_dMRI`'s own dependencies force `<=3.9.2` the moment it's installed, and since `matplotlib` isn't touched by the reconstruction math or the NRMSE test suite (only by `client/display.py`'s visualization), there's no reason to let pip silently downgrade it later instead of just starting there.
