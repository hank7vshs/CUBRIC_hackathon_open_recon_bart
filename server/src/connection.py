...
"""
connection.py — Low-level MRD protocol handler.

Wraps a TCP socket and implements the MRD message framing protocol:
read/write message identifier, optional length field, and typed payload
(config file, XML header, text, acquisitions, images, waveforms).

Public interface used by modules:
    Connection(socket, savedata, ...)  -- constructor
    __iter__ / __next__               -- iterate incoming MRD messages
    send_image(images)                -- send list of ismrmrd.Image
    send_close()                      -- send MRD_MESSAGE_CLOSE
    send_logging(level, contents)     -- send error/warning log text to client
"""
...
class Connection:
    ...
    def save_additional_config(self, ui_data_text):
        if self.savedata is True:
            if self.dset is None:
                self.create_save_file()

            self.dset._file.require_group("dataset")
            dset_ui_data = self.dset._dataset.require_dataset('configAdditional',shape=(1,), dtype=h5py.special_dtype(vlen=bytes))
            dset_ui_data[0] = bytes(ui_data_text, 'utf-8')
