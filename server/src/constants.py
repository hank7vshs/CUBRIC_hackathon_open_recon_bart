...
"""
constants.py — MRD message type identifiers and wire-format structs.

These constants define the integer identifiers used in the MRD TCP protocol
to tag each message (config, header, acquisition, image, waveform, etc.).
Entries marked DEPRECATED or UNSUPPORTED are kept for documentation only;
only the non-deprecated IDs are actively used by this server.
"""
