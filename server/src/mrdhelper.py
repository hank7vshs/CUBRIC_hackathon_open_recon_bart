...
"""
mrdhelper.py — Utility functions for working with MRD data.

Provides helpers for:
  - Copying AcquisitionHeader fields into an ImageHeader
  - Reading typed user parameters from the MRD XML header
  - Reading and writing ISMRMRD Meta Attributes
  - Parsing the ICE MiniHeader (scanner-internal binary header)
  - Reading UI parameters from the Open Recon JSON (ui_data dict)
  - Creating ROI annotations for display at the scanner
"""
...
def get_json_param(config, field, default=None, cast=None, **kwargs):
    """
    Read a parameter from JSON config.

    Args:
        config  : dict of parameters (the parsed ui_data JSON object)
        field   : name (key) of the parameter inside config['parameters']
        default : value returned when the field is absent or config is invalid
        cast    : optional type coercion — one of 'int', 'float', 'string', 'bool'
                  (also accepts 'double', 'str', 'choice', 'boolean' as aliases).
                  When None the raw value is returned unchanged.

    Returns:
        The parameter value, coerced to *cast* if specified, or *default* if absent.

    Note:
        The legacy keyword argument ``type`` is accepted as an alias for ``cast``
        for backwards compatibility, but ``cast`` is preferred to avoid shadowing
        the Python built-in.
    """
    # Accept legacy 'type=' keyword for backwards compatibility
    if cast is None and 'type' in kwargs:
        cast = kwargs.pop('type')
    ...
    if cast is None:
        ...
        return int(value)
    elif (cast == 'float') or (cast == 'double'):
        return float(value)
    elif (cast == 'string') or (cast == 'str') or (cast == 'choice'):
        return str(value)
    elif (cast == 'bool') or (cast == 'boolean'):
        ...
        raise Exception("'cast' must be int, float, string, or bool")
