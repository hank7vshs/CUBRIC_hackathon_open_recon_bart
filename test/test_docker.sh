...
source "${MAIN_DIR}/server/opts.sh"
...
run_test()
{
    ...
    # Ensure any leftover container from a previous run is fully stopped and removed
    # before starting the new one, so the port is guaranteed free
    source "${MAIN_DIR}/server/opts.sh"
    ...
    (
        ...
        ./run_docker.sh "${OR_MODULE}" > /dev/null 2>&1
    ) &
    PID1=$!
    ...
    (
        ...
        # Wait until the server is ready by watching docker logs for the "Serving" message.
        # Using /dev/tcp probes would consume the server's single accept() call.
        echo "Waiting for server on port 9002..."
        TIMEOUT=30
        ELAPSED=0
        until docker logs "${IMAGE_NAME}" 2>&1 | grep -q "Serving"; do
            sleep 1
            ELAPSED=$((ELAPSED + 1))
            if [ $ELAPSED -ge $TIMEOUT ]; then
                echo "Timeout: server did not become ready after ${TIMEOUT}s"
                exit 1
            fi
        done
...
# Initial build of docker file, because might take some time otherwise
# Client is limited to a couple of second waiting
